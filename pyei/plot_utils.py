...
def plot_precincts(
    ...
    if N > max_precincts and not show_all_precincts:
        warnings.warn(
            f"User attempted to plot {N} precinct-level voting preference "
            f"ridgeplots. Automatically restricting to first {max_precincts} precincts "
            f"(run with `show_all_precincts=True` to plot all precinct ridgeplots.)"
        )
        voting_prefs = [prefs[:, 2:max_precincts] for prefs in voting_prefs]
        if precinct_labels is not None:
            precinct_labels = precinct_labels[2:max_precincts]
        N = max_precincts-2
